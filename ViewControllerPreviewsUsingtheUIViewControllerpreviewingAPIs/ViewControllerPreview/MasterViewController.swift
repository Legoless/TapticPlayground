...
class MasterViewController: UITableViewController {
    ...
    @IBAction func vibrateButtonTap(sender: UIBarButtonItem)
    {
        if sender.title != "Stop" {
            sender.title = "Stop"
            
            self.timer = NSTimer.scheduledTimerWithTimeInterval(0.5, target: self, selector: "vibrate", userInfo: nil, repeats: true)
        }
        else {
            sender.title = "Vibrate"
